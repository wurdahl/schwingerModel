...
#Pauli-Villars ratio as a single pseudofermion: the two-flavour DWF weight
#det(D(m) D(m)^dag)/det(D(1) D(1)^dag) is generated by
#   S_f = phi^dag D(1)^dag (D(m) D(m)^dag)^-1 D(1) phi,   phi = D(1)^-1 D(m) chi,
#so the heat bath gives S_f = chi^dag chi. In schur mode D -> S throughout:
#det(D(m))/det(D(1)) = det(S(m))/det(S(1)) exactly because the even-even
#block c = _schurDiag is mass independent.
...
def pseudoBilinear(settings, pseudoField, gaugeLinks, cgRtol, schur=_SCHUR_DEFAULT):
    ...
    if schur:
        ...
        #action is (S1 phi_o)^dag (S S^dag)^-1 (S1 phi_o). Same variational CG
        #structure as the unpreconditioned normal equations (the bilinear is
        #quadratically accurate in the residual), just on the smaller,
        #better-conditioned S.
        rhs = applySchur(pvSettings, gaugeLinks, pseudoField, +1)
        A = lambda v: applySchur(settings, gaugeLinks,
                                 applySchur(settings, gaugeLinks, v, -1), +1)
        X, _ = cg(A, rhs, tol=cgRtol)
        relRes = jnp.linalg.norm(A(X) - rhs) / jnp.linalg.norm(rhs)
        return jnp.vdot(rhs, X).real, relRes

    rhs = applyD_dwf(pvSettings, gaugeLinks, pseudoField)
    operator = lambda v: applyD_dwf(settings, gaugeLinks, applyDdag_dwf(settings, gaugeLinks, v))

    X, _ = cg(operator, rhs, tol=cgRtol)
    relRes = jnp.linalg.norm(operator(X) - rhs) / jnp.linalg.norm(rhs)
...
def hmcForcingFunction_vec(settings: dwfParams, gaugeLinks, phis, x0=None, cgRtol=1e-5,
    ...
    if schur:
        #preconditioned ratio action: phis is the packed odd-site phi_o, and the
        #force comes from the single Hermitian solve X = (S S^dag)^-1 S1 phi_o.
        #One variational CG keeps the c64 forward error at the same level as
        #the unpreconditioned normal equations (chaining two nonsymmetric Schur
        #solves instead lets c64 rounding amplify through kappa(D): ~30% force
        #error and zero acceptance at m=0.01 Nx=64).
        rhs = applySchur(pvSettings, gaugeLinks, phis, +1)
        A = lambda v: applySchur(settings, gaugeLinks,
                                 applySchur(settings, gaugeLinks, v, -1), +1)
        X, _ = cg(A, rhs, tol=cgRtol, x0=x0)
        Y = applySchur(settings, gaugeLinks, X, -1)
        warmStart = X
    else:
        rhs = applyD_dwf(pvSettings, gaugeLinks, phis)
        operator = lambda v: applyD_dwf(settings, gaugeLinks, applyDdag_dwf(settings, gaugeLinks, v))

        X, _ = cg(operator, rhs, tol=cgRtol,x0=x0)
    ...
    # --- Fermion force for the ratio action S_f = (D1 phi)^dag (D D^dag)^-1 (D1 phi):
    #   dS_f = 2Re<X| dD1 |phi> - 2Re<X| dD |Y>
    # (the +phi term from differentiating D1 phi, the -Y term from the inverse).
    # Unpreconditioned, dD1 = dD (the mass carries no link), so it is one
    # bilinear with right = phi - Y. In schur mode dS1 != dS because D_eo
    # carries the mass, so the two terms use their own settings.
    Force = jnp.stack([guageTimeForce, gaugeSpaceForce], axis=-1)

    if schur:
        #schurFermionForce(s, X, V) returns -2Re<X| dS(s) |V>
        Force += (schurFermionForce(settings, gaugeLinks, X, Y)
                  - schurFermionForce(pvSettings, gaugeLinks, X, phis))
    else:
        Force += fermionForceBilinear(settings, gaugeLinks, X, phis - Y)
...
def hmcStep(settings:dwfParams, gaugeLinks, rngKey,  numSubSteps=100, cgRtolForce=1e-5,cgRtolAction=1e-10,
    ...
    kChi, kMom, kMetro = jax.random.split(rngKey, 3)
    ...
    #ratio heat bath phi = D1^-1 D chi, computed as D1^dag (D1 D1^dag)^-1 (D chi):
    #the fMass=1 operator is well conditioned and the action is then exactly
    #chi^dag chi. In schur mode the pseudofermion lives on the odd checkerboard
    #only, phi_o = S1^-1 S chi_o, and det(S S^dag)/det(S1 S1^dag) equals
    #det(D D^dag)/det(D1 D1^dag) because the even-even block c is mass
    #independent, so the sampled ensemble is identical. Tight rtol because the
    #heat-bath error is not corrected by the Metropolis step.
    pvSettings = settings._replace(fMass=1.0)
    if schur:
        chi = packParity(settings, chi, 1)
        b = applySchur(settings, gaugeLinks, chi, +1)
        A1 = lambda v: applySchur(pvSettings, gaugeLinks,
                                  applySchur(pvSettings, gaugeLinks, v, -1), +1)
        y, _ = cg(A1, b, tol=cgRtolAction)
        resHB = jnp.linalg.norm(A1(y) - b) / jnp.linalg.norm(b)
        phi = applySchur(pvSettings, gaugeLinks, y, -1)
    else:
        b = applyD_dwf(settings, gaugeLinks, chi)
        DDdag_pv = lambda v: applyD_dwf(pvSettings, gaugeLinks, applyDdag_dwf(pvSettings, gaugeLinks, v))
        y, _ = cg(DDdag_pv, b, tol=cgRtolAction)
        resHB = jnp.linalg.norm(DDdag_pv(y) - b) / jnp.linalg.norm(b)
        phi = applyDdag_dwf(pvSettings, gaugeLinks, y)

    #initial fermion action is just \chi.\chi
    initialFermionAction = jnp.vdot(chi,chi).real

    #first momentum half step:
    Force, X = hmcForcingFunction_vec(settings, gaugeLinksCopy,phi,cgRtol=cgRtolForce,
                                      schur=schur)
    ...
    def leapfrogBody(carry, _):
        ...
        F, Xnew = hmcForcingFunction_vec(settings, U, phi,
    ...
    Force, X = hmcForcingFunction_vec(settings,gaugeLinksCopy,phi,
    ...
    worstRes = jnp.maximum(resHB, resFinal)
